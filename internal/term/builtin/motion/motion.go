	...
	return command.Module{
		...
		Commands: []command.Command{
			...
			},
			{
				...
				Modes:     command.DocNormalModes,
				Keys:      kit.Keys(kit.Tab, kit.Ctrl('i')),
			},
			{
				...
				Modes:     command.DocNormalModes,
				Keys:      kit.Keys(kit.Shift(kit.Tab), kit.Ctrl('o')),
			},
			{
				Name:      actSaveSelection,
				DocString: "Push current selection onto jumplist",
		...
		Labels: []command.PrefixLabel{
			kit.Label(
				"Goto", kit.Char('g'), command.CmdKeyModes,
			),
		},
	}
}
