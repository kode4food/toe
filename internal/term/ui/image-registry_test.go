...
func TestImageDisplayNoStarvation(t *testing.T) {
	...
	real := geom.Size{Width: 100, Height: 50}
	...
	cmd2 := r.display(displayArgs{img: img, path: "x", id: 7, cells: real})
	...
	cmd3 := r.display(displayArgs{img: img, path: "x", id: 7, cells: real})
	assert.NotNil(t, cmd3, "the suppressed size must be retried, not lost")
	assert.Equal(t, real, r.placed[7])
}
